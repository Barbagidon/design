...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_calc__WEBPACK_IMPORTED_MODULE_6__["default"])('#size', '#material', '#options', '.promocode', '.calc-price', calcState, '.calc_form');
...
/* harmony import */ var _services_requests__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./services/requests */ "./src/js/modules/services/requests.js");



var calc = function calc(size, material, options, promo, price, state, calcFormSelector) {
  ...
  function calcPrice() {
    ...
    function createState(block, state, stateKey) {
      if (block === promoBlock) {
        ...
        var _options = block.options;
        var index = block.selectedIndex;
        state[stateKey] = _options[index].textContent.replace(/[\t\n]/gi, '');
      }
  ...
  function sendData(state) {
    var data = new FormData(calcForm);

    for (var key in state) {
      data.append(key, state[key]);
    }

    Object(_services_requests__WEBPACK_IMPORTED_MODULE_1__["postData"])('assets/question.php', data).then(function (res) {
      console.log(res);
...
var drop = function drop() {
  ...
  ['drageneter', 'dragover'].forEach(function (action) {
    fileInputs.forEach(function (input) {
      input.addEventListener(action, function () {
        return addStyle(input);
      });
    });
  });
  ['dragleave', 'drop'].forEach(function (action) {
    fileInputs.forEach(function (input) {
      input.addEventListener(action, function () {
        return removeStyle(input);
      });
    });
  });
  ...
  function createMessage() {
    forms.forEach(function (form) {
      form.addEventListener('submit', function (e) {
        ...
        var makeFormContent = function makeFormContent(src, text) {
        ...
        Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["postData"])(link, data).then(function (res) {
          console.log(res);
...
var modals = function modals() {
  ...
  function modalContent(trigger, item, closeTriggerSelector, closeBackSelector) {
    ...
    function triggerJump() {
      var gift = document.querySelector('.fixed-gift');
      gift.style.right = "".concat(+getComputedStyle(gift).right.replace(/\D/ig, '') + scroll, "px");
    }
    ...
    showModalByTime('.popup-consultation'); // document.body.addEventListener('click', (e) => {
    //     console.log(e.target);
    // });
  }
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImage(imageSelector) {
    ...
    imageSelector.querySelectorAll('p:not(.sizes-hit)').forEach(function (item) {
      item.style.display = 'none';
    });
  }
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var scrolling = function scrolling(upSelector) {// const element = document.documentElement, //возвращает весь документ
  //     body = document.body; // возвращает body
  // const calcScroll = () => {
  //     upElem.addEventListener('click', function (e) {
  //         let scrollTop = Math.round(body.scrollTop || element.scrollTop); //Сколько юзер отлистал(записывается по клику)
  //         console.log(scrollTop);
  //         if (this.hash !== "") {
  //             e.preventDefault();
  //             let hashElement = document.querySelector(this.hash),///нудный элемент
  //                 hashElementTop = 0; /// расстояние от нужного элемента до верха страницы.
...
var showMoreStyles = function showMoreStyles(trigger, wrapperSelector) {
